...
<div class="order-summary-container">
                ...
                <tr>
                    <td class="product-info">
                        <div class="d-flex align-items-center">
                            ...
                            @endif
                            <span class="product-name">{{ $item->link->product->product_name }}</span>
                        </div>
                    </td>
                    <td class="text-end">Rs {{ number_format($item->order->cost / $item->order->quantity, 2) }}</td>
                    <td class="text-center">
                        <span class="quantity-badge">{{ $item->order->quantity }}</span>
                    </td>
                    <td class="text-end price-column">Rs {{ number_format($item->order->cost, 2) }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <div class="order-totals">
        <div class="row">
            <div class="col-md-8">
                @if($orderData['order']->tracking_number || $orderData['order']->tracking_link)
                <div class="tracking-info">
                    @if($orderData['order']->tracking_number)
                    <div class="tracking-detail">
                        <strong>Tracking Number:</strong> 
                        <span class="tracking-number">{{ $orderData['order']->tracking_number }}</span>
                    </div>
                    @endif
                    @if($orderData['order']->tracking_link)
                    <div class="tracking-detail mt-2">
                        <strong>Tracking Link:</strong> 
                        <a href="{{ $orderData['order']->tracking_link }}" target="_blank" class="tracking-link">
            ...
            <div class="col-md-4">
                <table class="table table-borderless">
                    <tr>
                        <td>Subtotal:</td>
                        <td class="text-end">Rs {{ number_format($orderData['order']->total_cost - 300, 2) }}</td>
                    </tr>
                    <tr>
                        <td>Delivery Fee:</td>
                    ...
                    <tr class="fw-bold">
                        <td>Total:</td>
                        <td class="text-end">Rs {{ number_format($orderData['order']->total_cost, 2) }}</td>
                    </tr>
                </table>
            </div>
        </div>
            ...
            <div class="progress-wrapper">
                ...
                <div class="progress-steps">
                    ...
                    @foreach($statuses as $status => $label)
                        ...
                        <div class="progress-step 
                            @if($isCompleted && !$isActive) step-completed 
                            @elseif($isActive) step-active 
                            @else step-pending 
                            @endif">
                ...
                <div class="text-center mt-4">
                    <div class="alert alert-info" style="background: linear-gradient(135deg, #e6f3ff 0%, #cce7ff 100%); border: 1px solid #66b3ff; color: #0066cc;">
                        <strong>Current Status:</strong> {{ $statuses[$currentStatus] ?? $currentStatus }}
                        <br>
                        <small>Order Code: <strong>{{ $orderData['order']->order_code }}</strong></small>
                        <br>
                        <small>Order Date: <strong>{{ $orderData['order']->created_at->format('M d, Y h:i A') }}</strong></small>
                    </div>
                </div>
            </div>
        </div>
